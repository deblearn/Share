...
\title{Divide and Concur: a framework for dimensionality reduction and low dimensional embedding }
...
\begin{abstract}
To embed high dimensional data into low dimensional space by retaining intrinsic structure of data is a great challenge and long term study from very earlier age. Several linear and non-linear embedding methods  are available to solve this problem where each method has  their own objective functions. In this paper, our aim is to represent non linear embedding methods under one constraint satisfaction problem(CSP) framework. We use Divide and Concur approach(an efficient method to solve constraint satisfaction problem) to build up our platform where different objective functions will be satisfied by using one framework.           
